...
pub_subnet=$(aws ec2 create-subnet --availability-zone-id $az --cidr-block "10.0.2.0/24" --vpc-id $vpc_id)

# Create a new private subnet in the VPC
priv_subnet=$(aws ec2 create-subnet --availability-zone-id $az --cidr-block "10.0.12.0/24" --vpc-id $vpc_id)
